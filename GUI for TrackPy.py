...
# Create a class for tip widget
# Ref: https://stackoverflow.com/questions/20399243/display-message-when-hovering-over-something-with-mouse-cursor-in-python
...
def save_csv():
    inte_loss_1 = pd.DataFrame(data ={"Time":range(tracks2['Frame'].max())})*sec_per_f
    inte_loss_1.set_index("Time", inplace=True)
...
labels = [
    "File:", "Red channel:", "FRET channel:", "Green channel:", "Adding saponin at:", 
    "Second per frame (sec):", "Diameter (nm):", "Minimum intensity:", "Separation:", "Percentile (%):", 
    "Max distance (\u03bcm):", "Memory (frame):", "Minimum length (frame):", "Background radius (pixel):", 
...
tips = [
        "Read Image files", 
        "Signal you want to show in red in the intensity plot", 
        "Not working yet",
        "Signal you want to show in green in the intensity plot", 
        "At which frame you add saponin into the well:",
        "Second per frame (sec)",
        "Diameter (nm) of particles. The number(s) must be odd integers. When in doubt, round up.",
        "The minimum integrated brightness. This is a crucial parameter for eliminating spurious particles.", 
        ...
        "Maximum distance in \u03bcm that features can move between frames to be considered the same feature.",
        "Maximum number of frames in which a feature can dissapear, \nthen reappear nearby, and be considered the same particle.",
        "Minimum number of points for a track to be considered.",
        "The size of background around particles for background subtraction.",
        "Amount of frames between two tracks for them to be considered as same track.",
...
CreateToolTip(file_label, text = tips[0])
...
for i in range(1, 17):
    ...
    CreateToolTip(label, text = tips[i])
...
CreateToolTip(saveplot_label, text = tips[17])
